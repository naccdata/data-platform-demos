"""Example script for pulling enrollment errors from a pipeline project on NACC
Data Platform."""
...
def main():
    """Pull enrollment errors from a pipeline project on the NACC Data Platform."""
    parser = argparse.ArgumentParser(
        description="Pull enrollment errors from pipeline project"
    )
    parser.add_argument(
        "-a",
        "--adcid",
    ...
    parser.add_argument(
        "-o",
        "--output",
        help="output file path (default: enrollment-errors-<project>-<date>.csv)",
        default=None,
    )
    parser.add_argument(
    ...
    output_path = args.output or filter_helpers.build_output_filename(
        "enrollment-errors", source_project.label, module_set, ptid_set
    )
