...
4. Reviewing submissions
   - [Listing submissions](https://github.com/naccdata/data-platform-demos/tree/main/demo/list_submissions) — browse all submissions in a project with their overall QC status
   - [Submission detail](https://github.com/naccdata/data-platform-demos/tree/main/demo/submission_detail) — view per-stage QC breakdown and errors for a specific submission

The error, status, and submission scripts support `--module` and `--ptid` flags for filtering results by module name or participant ID. See each demo's README for details.

A typical review workflow is to first run `list_submissions` to see which submissions have failed, then use `submission_detail` with the participant ID, date, and module to drill into the per-stage QC summary and export errors.
